...
#这是将mask-rcnn检测到的框裁剪下来,保存到对应的文件夹下
# import os
# def visualize_boxes_and_labels_on_image_array(
#         image,
...
#                 # for j in tf.unstack(instance_masks):
#
#                 box_to_instance_masks_map[box] = instance_masks[i]
#             if instance_boundaries is not None:
#                 box_to_instance_boundaries_map[box] = instance_boundaries[i]
...
#
#         image_ = image[new_ymin:new_ymax,
#                  new_xmin:new_xmax]  # 将测试图片做裁剪，只剩bounding box部分，注意此处的image_命名，之前我写的是image,会出错。而且要注意的是，由于代码中用的是PIL图片格式，原点在左上角，而此处用numpy的方式裁剪图片，原点在左下角，所以x和y是反过来的。
#
#         plt.imshow(image_)
#         plt.show()
#
#         PATH_TO_crop_DIR = '/home/ubutnu/work/dataArea/gxh-1/copy'  # 保存的裁剪图片位置
...
#         image_.save(os.path.join(PATH_TO_crop_DIR, (str(t) + '_') + os.path.basename(
#             image_path)))  # 这里用上了image_path,因为os.path.join需要字符串的参数，由于一张测试图片可能会检测出多个bounding box，所以在保存命名的时候加上了t这个参数（这一步应该会有更优雅的做法，但是我懒得去试了）
#     ####################################################################################
